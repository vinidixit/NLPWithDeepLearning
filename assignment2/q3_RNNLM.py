...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(tf.int32, [None, self.config.num_steps],'input')
    self.labels_placeholder = tf.placeholder(tf.float32, [None, self.config.num_steps], 'labels')
    self.dropout_placeholder = tf.placeholder(tf.float32, [None, self.config.num_steps], 'dropout')
    ### END YOUR CODE
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      L = tf.Variable(tf.random_uniform([len(self.vocab, self.config.embed_size)], -1, 1))
      inputs = tf.nn.embedding_lookup(L, self.input_placeholder)
      inputs = tf.split(inputs, num_or_size_splits=self.config.num_steps, axis=1)
      ### END YOUR CODE
      return inputs
