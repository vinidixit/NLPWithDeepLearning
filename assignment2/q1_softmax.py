...
def softmax(x):
  ...
  ### YOUR CODE HERE
  x = x - tf.reshape(tf.reduce_max(x, reduction_indices=[1]), [tf.shape(x)[0],1])
  out = tf.exp(x)/tf.reshape(tf.reduce_sum(tf.exp(x),reduction_indices=[1]), [tf.shape(x)[0], 1])
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  out = tf.reduce_sum(tf.multiply(tf.to_float(y),tf.log(tf.to_float(yhat))))*-1
  ### END YOUR CODE
  return out
