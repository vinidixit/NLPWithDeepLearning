...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    """
    ### YOUR CODE HERE
    #tf.Variable(tf.zeros([self.config.window_size*self.config.embed_size, self.config.hidden_size]))
    with tf.variable_scope('Layer'):
        shape = (self.config.window_size * self.config.embed_size, self.config.hidden_size)
        W = tf.get_variable(name='W', shape=shape, initializer=xavier_weight_init())
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))

    with tf.variable_scope('Softmax'):
        shape = (self.config.hidden_size, self.config.label_size)
        U = tf.get_variable(name='U', shape=shape, initializer=xavier_weight_init())
    ...
    out = tf.matmul(h, U)+b2
    output = tf.nn.dropout(out, self.dropout_placeholder)#tf.nn.softmax(tf.matmul(h_drop, U)+b2)
  ...
  def add_loss_op(self, y):
    ...
    with tf.variable_scope('Softmax', reuse=True):
        U = tf.get_variable('U')
    loss = loss + (self.config.l2/2) *(tf.reduce_sum(tf.square(W))+tf.reduce_sum(tf.square(U)))
    ### END YOUR CODE
    return loss
